...
function pairAndGetPin(baseUrl) {
...
async function main() {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'syncd-e2e-'));
  ...
  try {
    ...
    console.log('Sharing file via share.js...');
    ...
    console.log('share.js:', shareResult.stdout.split('\n').filter(Boolean).pop());

    const relName = path.basename(originalFile);
    let useCloud = false;
    if (storage.isStorageEnabled()) {
      console.log('Uploading to cloud storage synchronously...');
      const state = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'state.json'), 'utf8'));
      const masterKey = Buffer.from(state.masterKey, 'hex');
      const sharedFile = path.join(SHARED_DIR, relName);
      const entry = await storage.uploadFile(DATA_DIR, masterKey, relName, sharedFile);
      useCloud = !!entry;
      console.log(`Cloud upload ${useCloud ? 'completed' : 'failed'}`);
    }
    ...
    console.log(`   Transfer mode: ${useCloud ? 'Cloud + chunked AEAD' : 'local chunked AEAD'}`);
